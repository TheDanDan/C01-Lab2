    ...
    try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        if (!data) {
          return res
          .status(404)
          .json({ error: "Unable to find note with ID " + noteId });
        }
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
    try {
      ...
      if (!title && !content) {
        return res
          .status(400)
          .json({ error: "Title or content are required." });
      }

      // Basic param checking
      const noteId = req.params.noteId;
      if (!ObjectId.isValid(noteId)) {
        return res.status(400).json({ error: "Unable to find note with ID " + noteId });
      }
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        const newContent = content ? content: data.content;
        const newTitle = title ? title: data.title;
        ...
        res.json({ response: `Document with ID ${noteId} properly edited` });
      });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
